...
// import { sendEmailNodemailer } from "../services/nodemailer.service";
...
const AdminHandler = {
    ...
    async getUserDetails(userId: string): Promise<ApiResponse> {
        try {
            ...
            let getResponse = await findOne(adminModel, { _id: userId });

            if (!getResponse.status) {
                return showResponse(false, responseMessage.admin.invalid_admin, null, null, 400)
            }

            return showResponse(true, responseMessage.users.user_detail, null, null, 200)
